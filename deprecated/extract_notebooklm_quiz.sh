...
#
# USAGE:
#   ./extract_notebooklm_quiz.sh <path/to/saved_page_folder_or_file> [output.md]
#
# SUPPORTS:
...
if os.path.isdir(input_path):
    ...
    # Look for accompanying _files folder
...
# ── PART C: Build markdown output ─────────────────────────────────────────────

lines = []
lines.append("# NotebookLM Quiz Export\n")
lines.append(f"*Total questions extracted: {total_found}*\n")
lines.append("---\n")

# Interactive quiz
if quiz_questions:
    lines.append(f"## Part 1 — Interactive Quiz ({len(quiz_questions)} Questions)\n")
    lines.append("*Correct answers, rationales for all options, and hints included.*\n\n")
    ...
    for i, q in enumerate(quiz_questions, 1):
        ...
        for j, opt in enumerate(options):
            ...
            option_lines.append(f"**{letter}.** {opt_text}")
            marker = "✅" if is_right else "❌"
            rationale_lines.append(f"- {marker} **{letter}:** {rat}")
...
# Chat questions
if chat_questions:
    lines.append(f"\n## Part 2 — Chat Questions with Reasoning ({len(chat_questions)} Questions)\n")
    lines.append("*Includes stream-of-thought clinical reasoning and explanations.*\n\n")
    for block in chat_questions:
